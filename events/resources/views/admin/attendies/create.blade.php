...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.attendy.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.attendies.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('participant') ? 'is-invalid' : '' }}" name="participant_id" id="participant_id" required>
                    @foreach($all_participants as $id => $entry)
                        <option value="{{ $id }}" {{ old('participant_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
