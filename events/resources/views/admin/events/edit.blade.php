    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time') ? 'is-invalid' : '' }}" type="text" name="time" id="time" value="{{ old('time', $event->time) }}">
                @if($errors->has('time'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('venue') ? 'is-invalid' : '' }}" type="text" name="venue" id="venue" value="{{ old('venue', $event->venue) }}" required>
                @if($errors->has('venue'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('participants') ? 'is-invalid' : '' }}" name="participants[]" id="participants" multiple required>
                    @foreach($all_participants as $id => $participant)
                        <option value="{{ $id }}" {{ (in_array($id, old('participants', [])) || $event->participants->contains($id)) ? 'selected' : '' }}>{{ $participant }}</option>
                    @endforeach
                </select>
